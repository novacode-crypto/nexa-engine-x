...
class NexaEngineX {
  ...
  private async setupIpcHandlers(): Promise<void> {
    ...
    ipcMain.handle('set-setting', async (_, key: string, value: unknown) => {
      ...
      const validKeys = [
        'autoStart', 'startMinimized', 'minimizeToTray', 'autoUpdate',
        'downloadsPath', 'apiPort', 'apiToken', 'binariesPath',
        'lastDiagnostics', 'pluginRegistry'
      ] as const;
    ...
    ipcMain.handle('open-downloads-folder', async () => {
      const downloadsPath = this.store.get('downloadsPath', path.join(app.getPath('downloads'), 'YTDownloadX'));
